...
@click.option(
    ...
    default="self",
    help=(
        "Type of optimization to perform: self-refinement, example-based, or "
        "metric-based."
    ),
)
...
def main(
    ...
    if not api_key:
        click.echo(
            "Error: Anthropic API key is required. Provide it with --api-key or "
    ...
    if verbose:
        click.echo(
            f"Optimizing prompt using {optimization_type} approach with model "
            f"{model}...",
            err=True,
        )
